...
class MrcObjectTest(unittest.TestCase):
    ...
    def test_header_is_correct_for_3d_data(self):
        x, y, z = 4, 3, 2
        self.mrcobject.set_data(np.arange(z * y * x, dtype=np.int16).reshape(z, y, x))
    ...
    def test_header_is_correct_for_4d_data(self):
        x, y, z, nvol = 3, 4, 5, 6
        vstack = np.arange(nvol * z * y * x, dtype=np.int16).reshape(nvol, z, y, x)
    ...
    def test_switching_4d_data_to_image_stack_raises_exception(self):
        self.mrcobject.set_data(np.arange(24, dtype=np.int16).reshape(2, 2, 2, 3))
        with self.assertRaises(ValueError):
            self.mrcobject.set_image_stack()
    ...
    def test_5d_data_raises_exception(self):
        with self.assertRaises(ValueError):
            self.mrcobject.set_data(np.arange(2, dtype=np.int16).reshape(1, 1, 1, 1, 2))
    ...
    def test_header_byte_order_is_unchanged_by_data_with_native_order(self):
        ...
        assert utils.byte_orders_equal(
            header.mode.dtype.byteorder, data.dtype.byteorder
        )
        self.mrcobject.set_data(data)
        assert utils.byte_orders_equal(
            header.mode.dtype.byteorder, data.dtype.byteorder
    ...
    def test_setting_origin_as_separate_fields(self):
        ...
        expected = fromrecords((1.0, -5.0, 128.0), dtype=dtypes.VOXEL_SIZE_DTYPE)
        np.testing.assert_equal(self.mrcobject.header.origin, expected)

    def test_setting_origin_as_single_field(self):
        origin = fromrecords((-1.0, 1.0, 0.0), dtype=dtypes.VOXEL_SIZE_DTYPE)
